...
func renderArchitectureMap(modules []maps.ModuleInfo, importGraph map[string][]string, totalGoFiles, totalLOC int) string {
	...
	ts := timestampUTC()
	sb.WriteString(fmt.Sprintf("# Atlas System Architecture Map\n"))
	sb.WriteString(fmt.Sprintf("> Generated: %s | Modules: %d | Go Files: %d | Total LOC: %s\n\n",
		ts,
		len(modules),
		totalGoFiles,
		formatNum(totalLOC),
	...
	// Footer.
	sb.WriteString(fmt.Sprintf("_Generated by cmd/mapgen. Last updated: %s_\n", timestampUTC()))
