...
func renderArchitectureMap(modules []maps.ModuleInfo, importGraph map[string][]string, totalGoFiles, totalLOC int) string {
	...
	fmt.Fprintf(&sb, "> Generated: %s | Modules: %d | Go Files: %d | Total LOC: %s\n\n",
		ts, len(modules), totalGoFiles, formatNum(totalLOC))
...
func formatNum(n int) string {
	...
	for i, c := range s {
		...
		result = append(result, byte(c)) //nolint:gosec // c is ASCII digit (0-9), safe
	}
