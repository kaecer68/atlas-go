...
func registerEventTools(mcpSrv *mcp.Server, s *server) {
	countedAddTool(mcpSrv, &mcp.Tool{
		Name:        "event_calendar",
		Description: autoDescOr("event_calendar", "Upcoming Taiwan market events calendar (ETF rebalances, MSCI adjustments, revenue announcements, window dressing, holidays). 14-day forward look."),
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
	}, s.handleEventCalendar)

	countedAddTool(mcpSrv, &mcp.Tool{
		Name:        "event_flow_prediction",
		Description: autoDescOr("event_flow_prediction", "5-day event-driven capital flow prediction. Maps upcoming events (ETF rebalances, revenue, MSCI, etc.) to predicted capital flow directions by day with confidence scores."),
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
