...
type Config struct {
	...
	// means the emitter uses NoOpPublisher (no alert side-effects).
...
func Run(ctx context.Context, cfg Config) error {
	...
	// NoOpPublisher so the rest of the pipeline still runs in tests.
	anomalyAckStore := anomaly.NewMemoryStore(cfg.AnomalyStoreCapacity)
	var anomalyPub alerting.Publisher = &alerting.NoOpPublisher{}
