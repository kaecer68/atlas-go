...
// 資料來源：docs/ATLAS_CONSTITUTION_AUDIT.md（v1.1c, 2026-08-10, commit 8407f301 #1505）
//
...
var auditSnapshot = ConstitutionAuditState{
	...
	Governance: []GovernanceItem{
		...
		{ID: "F1", Group: "fmx", Title: "外資雙重動機模型（結構性 vs 投機性分流）", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已覆核", Note: "ForeignInvestorNet + ForeignDealerNet 欄位已存在（macro_provider.go）；scoreForeign 未消費投機性欄位（gap 已文件化，落地待回測驗證）"},
		{ID: "F2", Group: "fmx", Title: "自營商大小分流（大型可納宏觀，小型用 AI 分點）", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已覆核", Note: "DealerSelfNet + DealerHedgingNet 欄位已存在；scoreDealer 只用合計（gap 已文件化，AI 分點待資料源）"},
		{ID: "F3", Group: "fmx", Title: "投信主動 vs 被動分流（ETF 被動買盤 vs 主動基金）", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已覆核", Note: "ETFNetSubscription 資料源已移除（TWT44U → 404，2026-08-10 實測）；F3 淨化訊號暫不可行，見 docs/specs/institutional-flow-split-gap-analysis.md"},
		{ID: "F4", Group: "fmx", Title: "公股分點追蹤作為 BK-13 替代方案", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已覆核", Note: "GovernmentNet 已由 scoreGovernment 消費（每日總額形式）；分點層級待 BK-13/14 資料源（seam 已備，非程式 gap）"},
		{ID: "F5", Group: "fmx", Title: "選股層策略庫設計（Phase 4）", Status: AuditStatusNotStart, Versioned: "⬜ → ⬜", Note: "仍待 T27（v1.1c 唯一剩餘 ⬜）"},
		// M1-M6 MCP 工具對齊（群組 mcp）
		{ID: "M1", Group: "mcp", Title: "時期判斷 MCP 工具公開", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已覆蓋", Note: "macro_get_snapshot_latest.current_period + current_period_name_zh 已公開七時期欄位；#1488 註記不重複實作獨立 PeriodDetector 工具（避免 API 冗餘）", Related: "macro_get_snapshot_latest / macro_get_snapshot_history / macro_get_stress_index_current"},
		...
		{ID: "M4", Group: "mcp", Title: "策略適用時期 MCP 工具公開", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已實現", Note: "strategy_for_period 工具（PR #1488）：輸入 market period ID 回傳 allowed strategies + brief；讀 configs/methodology_rules.yaml 同源 MethodologyAdvisor", Related: "strategy_for_period / get_recommendations / strategy_list_active / strategy_ranker"},
		...
		{ID: "X1", Group: "enforce", Title: "PR 合併前憲章對齊檢查（CI gate）", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已實現", Note: "check_constitution.sh + check_methodology_constitution.sh + check_constitution_drift.sh 三個都在 make ci-gate（pre-push 強制）與 GitHub constitution.yml job", Related: "make ci-gate / check_constitution.sh"},
		{ID: "X2", Group: "enforce", Title: "方法論變更強制更新追蹤表", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已實現", Note: "PR template 加「方法論變更 → 同步 §附錄 F」checkbox + #1464 ACI hook 擴充（soft，不 block）", Related: "PR template checkbox / ACI soft reminder"},
		{ID: "X3", Group: "enforce", Title: "憲章漂移自動警報（nightly scan）", Status: AuditStatusDone, Versioned: "⬜ → ✅ 已實現（pre-push 形式）", Note: "check_constitution_drift.sh 已在每次 make ci-gate（pre-push）執行（比 nightly 更頻繁）；殘留：無獨立 nightly schedule job（可選，非阻塞）", Related: "check_constitution_drift.sh / traps.md"},
	},
	Stats: AuditStats{
		Total:              22,
		Done:               22,
