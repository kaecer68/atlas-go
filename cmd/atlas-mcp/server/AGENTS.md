...
- `server.Run()` 在所有 tool 註冊完成後 assert `RegisteredToolCount` 在 **89–91** 範圍內；**若不在範圍內直接 return error 阻止啟動**，防止文件↔程式碼漂移。此範圍對應 `docs/REFERENCE/tool-catalog.md` 所列對 agent 暴露的 tool 總數。
