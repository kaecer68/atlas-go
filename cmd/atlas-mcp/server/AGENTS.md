...
- 目前 `registerTools()` 內掛載的業務 tool handler 數量為 **97 個**（sampling / elicitation 兩個 feature-gated tool 關閉時）；`SamplingEnabled` / `ElicitationEnabled` 各再 +1，最高 **99 個**。
- `registerAuditTools()` 另外掛載 **4 個**自我觀測 tool，不計入 `registerTools()` 的範圍。
- `server.Run()` 在所有 tool 註冊完成後 assert `RegisteredToolCount` 在 **101–103** 範圍內；**若不在範圍內直接 return error 阻止啟動**，防止文件↔程式碼漂移。此範圍對應 `docs/REFERENCE/tool-catalog.md` 所列對 agent 暴露的 tool 總數。
