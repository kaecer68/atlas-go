...
| 觀測 | Rate limiting（`ratelimit.go`）、Metrics（`metrics.go`）、Anomaly detection（`tools_anomaly.go`）、MCP 自我觀測（`audit_state`、`mcp_get_call_stats` / `session_topology` / `top_slow_tools` / `tenant_usage`、`mcp_anomaly_*`） |
| 工具分類 | Regime（1）、Macro（6）、Capital Flow（3）、Crossmarket（3）、Narrative（8）、Events（2）、Risk（5）、Alert（8）、Strategy（7）、Recommendation（1）、Experiment（5）、Synergy（3）、Control（7）、Scheduler/Task（4）、System（7）、LLM（2）、Trace（4）、Data（4）、Stock（5）、Parameters（5）、Backtest（2）、Industry Extension（3）、Sector Canonical（2）、Universe（3）、Report（4）、Briefing（2）、Protocol Extensions（4，其中 2 個 feature-gated 預設關閉）、MCP Audit / Observability（7）、Template Detector（2） |

> **內部 observability（不需 backend）**：`audit_state` 與 `mcp_get_*` / `mcp_anomaly_*` 讀本地 audit log / embedded snapshot，backend（:18080）未啟動也可用；`system_get_maturity` 在 backend 不可達時降級回 embedded `internal/MATURITY.md` 快照。
> **已知限制（D-05）**：stdio transport 目前不會注入 agent identity，`AgentIDFromContext` 回 `anonymous`——`mcp_get_session_topology` 的 agent 維度會顯示為 `anonymous`。SSE / streamable-HTTP transport（Phase 4）接上 authenticated agent identity 後解除。
