...
> **Agent 入門** — 第一次使用？先讀 [`docs/AGENT_ONBOARDING.md`](../../docs/AGENT_ONBOARDING.md)（5 分鐘速讀），再看 [`docs/AGENT_TOOLS.md`](../../docs/AGENT_TOOLS.md)（74 個 tool 的決策樹與完整 catalog）。
> **完整規格** — 70+ tool 的設計文件、安全邊界、JSON Schema 模板見 [`docs/specs/agent-mcp-server.md`](../../docs/specs/agent-mcp-server.md)。
...
| 面向 | 現狀 |
|------|------|
| MCP Tools | **84 個**（19 個 `tools_*.go` 檔案，按 WA workflow area 分群） |
...
| 工具分類 | Macro（6）、Crossmarket（3）、Narrative（7）、Risk（5）、Alert（4）、Strategy（5）、Experiment（3）、Synergy（3）、Control（4）、Scheduler/Task（4）、System/Health（12+）、Data（4）、LLM（2）、Trace（4）、PRISM/Swarm（6）、Report（4） |
