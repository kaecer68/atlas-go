...
func TestRootPathRedirectsToAdmin(t *testing.T) {
	mux := http.NewServeMux()
	registerSimpleRoutes(mux, &monitoring.MetricsCollector{}, nil, nil, nil)
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("GET /: got status %d, want %d", rr.Code, http.StatusTemporaryRedirect)
	}
	if loc := rr.Header().Get("Location"); loc != "/admin/" {
		t.Errorf("GET /: got Location %q, want %q", loc, "/admin/")
	}
}
