...
- 維持可稽核性：mutation 決策與 promote 歷程需可追溯。
- 避免靜默漂移：若調整接受門檻或 judge profile，需在程式註解或 PR 說明原因。
- 維持控制層順序：除非有明確需求，control 過濾需維持在上游建議之後。
...
若變更影響 orchestration 或 simulation 邊界：
...
- `docs/experiment-baseline-report.md`：稀疏資料教訓與門檻調整背景
