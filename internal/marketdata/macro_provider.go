...
func (s MacroDataSnapshot) MarshalJSON() ([]byte, error) {
	...
	aux := &struct {
		*Alias
		...
		MarginMaintenanceRatio *MacroDataPoint `json:"margin_maintenance_ratio,omitempty"`
	...
	if s.MarginMaintenanceRatio.Symbol != "" {
		aux.MarginMaintenanceRatio = &s.MarginMaintenanceRatio
	}
...
func mergeSnapshot(dst *MacroDataSnapshot, src MacroDataSnapshot) {
	...
	if src.MarginMaintenanceRatio.Symbol != "" {
		dst.MarginMaintenanceRatio = src.MarginMaintenanceRatio
	}
