...
func TestFinMindProbeDate(t *testing.T) {
	cases := []struct {
		...
		{"教師節休市日的週一 09-28 → 前一個交易日（週末與中秋都須再往前）", "2026-09-28T09:01:00+08:00", "2026-09-24"},
		{"雙十連假 10-09（週五休市）→ 前一個交易日", "2026-10-09T09:01:00+08:00", "2026-10-08"},
		{"UTC 01:01（台北當日 09:01）→ 前一個交易日", "2026-09-24T01:01:00Z", "2026-09-23"},
		// 這一列是「必須以台北時區計算」的牙齒：2026-10-12T17:30Z = 台北 10-13 01:30
		// → 前一個交易日 2026-10-12（週一）；若改回用 UTC 時鐘（去掉 .In(taipeiLoc)），
		// UTC 日期 10-12 會往前走到 10-11（週日）→ 10-10／10-09（國慶連假休市）
		// → 2026-10-08，本列即 FAIL。
		{"UTC 傍晚（台北已跨日）→ 前一個交易日（時區不可省）", "2026-10-12T17:30:00Z", "2026-10-12"},
	}
