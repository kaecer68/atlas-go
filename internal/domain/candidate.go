...
type Candidate struct { Agent AgentSpec; Scorecard Scorecard; Experiment ExperimentRecord }
...
func SelectWeakestAgentExcluding(registry AgentRegistry, scorecards []Scorecard, extinctAgentIDs map[string]bool) *Candidate {
	if len(scorecards) == 0 { return nil }
	byID := make(map[string]AgentSpec, len(registry.Agents))
	for _, a := range registry.Agents { byID[a.ID] = a }
	ord := slices.Clone(scorecards)
	slices.SortFunc(ord, func(a, b Scorecard) int {
		aA, aO := byID[a.AgentID]; bA, bO := byID[b.AgentID]
		aP := clp(aA, aO); bP := clp(bA, bO)
		switch {
		case aP < bP: return -1; case aP > bP: return 1
		case a.WindowCount < b.WindowCount: return -1; case a.WindowCount > b.WindowCount: return 1
		case a.SharpeLike < b.SharpeLike: return -1; case a.SharpeLike > b.SharpeLike: return 1
		default: return 0
	}})
	for _, sc := range ord {
		ag, ok := byID[sc.AgentID]
		if !ok || !ag.Enabled { continue }
		if extinctAgentIDs != nil && extinctAgentIDs[ag.ID] { continue }
		sc.Layer = ag.Layer
		mt, hp, gt := smt(ag, sc)
...
func SelectBestSpawnedAgent(registry AgentRegistry, scorecards []Scorecard, spawnedAgentIDs map[string]bool, baselineSharpe float64) *Candidate {
	if len(scorecards) == 0 || len(spawnedAgentIDs) == 0 { return nil }
	byID := make(map[string]AgentSpec, len(registry.Agents))
	for _, a := range registry.Agents { byID[a.ID] = a }
	var best *Candidate
	for _, sc := range scorecards {
		ag, ok := byID[sc.AgentID]
		if !ok || !ag.Enabled || !spawnedAgentIDs[ag.ID] { continue }
		if sc.SharpeLike <= baselineSharpe { continue }
...
func pidf(exp ExperimentRecord) string {
	if exp.ProposalID != "" { return exp.ProposalID }
	if exp.ID != "" { return "proposal-"+exp.ID }; return ""
}
func clp(a AgentSpec, ok bool) int {
	if !ok { return 99 }
	switch a.Layer { case LayerSector, LayerStyle: return 0; case LayerContext: return 1; case LayerControl: return 2; default: return 3 }
}
func cml(w int) string {
	switch { case w>=5: return "level_3_regime_aware"; case w>=3: return "level_2_window_validated"; default: return "level_1_exploratory" }
}
func crw(w int) string {
	switch { case w>=5: return "next cross-regime replay window"; case w>=3: return "next multi-session replay window"; default: return "next short validation window" }
}
func cig(layer AgentLayer, w int) []string {
	...
	switch layer {
	case LayerSector: g = append(g, "Refine sector thesis.","Prefer industry-specific evidence.")
	case LayerStyle: g = append(g, "Tighten entry filters.","Do not smuggle sector policy into style mutation.")
	case LayerContext: g = append(g, "Adjust regime interpretation carefully.","Prefer threshold refinement.")
	case LayerControl: g = append(g, "Conservative governance changes.","Do not widen risk limits without evidence.")
	}
	if w < 3 { g = append(g, "Evidence thin; prefer conservative tightening.")
	} else { g = append(g, "Evidence adequate for focused replay challenge.") }
	return g
}
func smt(ag AgentSpec, sc Scorecard) (string, string, []string) {
	s, w, l := sc.SharpeLike, sc.WindowCount, ag.Layer
	if l == LayerControl {
		if w >= 5 && s < 0 { return "portfolio_constraint_revision", "Portfolio governance needs adjustment.", []string{"improve_sharpe_like","reduce_concentration_risk","maintain_cro_authority"} }
	...
	if l == LayerSector && w >= 4 && s < 0.1 { return "risk_rule_change", "Sector criteria need stronger thresholds.", []string{"improve_sharpe_like","reduce_sector_blindspots","maintain_industry_coverage"} }
	if l == LayerStyle && w >= 4 && s < 0.1 { return "risk_rule_change", "Style filters should be raised.", []string{"improve_sharpe_like","reduce_style_drift","maintain_momentum_catch"} }
