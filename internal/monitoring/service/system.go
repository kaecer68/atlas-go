...
func (s *SystemService) LoadSystemHealth() (SystemHealthResponse, error) {
	...
	channels := []DataChannelInfo{
		...
		buildChannelInfo("twse_margin", "TWSE 融資融券", checkCapitalFlowHealth, filepath.Join(s.WorkDir, "data/state/margin"), now),
		buildChannelInfo("export_statistics", "台灣海關進出口", checkExportHealth, filepath.Join(s.WorkDir, "data/state/export"), now),
		buildChannelInfo("tsmc_revenue", "台積電月營收", checkCapitalFlowHealth, filepath.Join(s.WorkDir, "data/state/tsmc_revenue"), now),
