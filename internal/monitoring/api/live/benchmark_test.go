...
func TestStdDev(t *testing.T) {
	tests := []struct {
		...
		{"two identical", []float64{2.0, 2.0}, 0},
		{"two different", []float64{0.0, 2.0}, 1.4142135623730951},
		{"three values", []float64{1.0, 2.0, 3.0}, 1.0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			} else if got < tt.want-0.0001 || got > tt.want+0.0001 {
...
func TestSessionDateFromID(t *testing.T) {
	tests := []struct {
		...
		{"valid session ID", "session-20260413-daily", time.Date(2026, 4, 13, 0, 0, 0, 0, time.UTC)},
		{"valid session ID v2", "session-20260522-batch", time.Date(2026, 5, 22, 0, 0, 0, 0, time.UTC)},
		{"no prefix", "20260413-daily", time.Time{}},
		{"invalid date", "session-notadate-daily", time.Time{}},
		{"empty string", "", time.Time{}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := sessionDateFromID(tt.id)
			if !got.Equal(tt.want) {
				t.Errorf("sessionDateFromID(%q) = %v, want %v", tt.id, got, tt.want)
			}
		})
	}
}
