...
| **零值 P/E 或 P/B 篩選掉所有標的** | 若 `ScreeningCriteria.PE` 或 `PB` 為 `0`，預設比較器 `>` 使用 `CriteriaValue=0`。所有 PE 值 >0，這會讓**所有**標的**通過**（`0` 表示「無篩選」）。在依賴篩選前請確認預期行為。參見根 `agents.md`「高危陷阱」中 `ScreeningCriteria` 說明。 |
