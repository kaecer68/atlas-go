...
func EstimateGumbelTheta(u, v []float64) (float64, error) {
	...
	// Perfect concordance: τ ≈ 1 → θ → ∞; clamp to a finite cap.
	if 1.0-tau < tauZeroEpsilon {
		return maxThetaClamp, nil
	}
	theta := 1.0 / (1.0 - tau)
	// Guard numerical drift: any τ < 0 would push θ below 1, which is
	// outside the Gumbel domain. Snap to the boundary.
	if theta < 1.0 {
		return 1.0, nil
	}
	return theta, nil
...
func EstimateClaytonTheta(u, v []float64) (float64, error) {
	...
	// Floor at the Clayton domain boundary (τ ≤ -1 case is clamped at -1).
	if theta < -1.0 {
		return -1.0, nil
	}
	return theta, nil
...
// validatePseudoObservations checks the preconditions shared by both
// estimators: same length, finite, in [0,1], non-degenerate variance.
