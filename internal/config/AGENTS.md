...
| **Magic number 繞過參數系統** | 禁止在業務邏輯中硬編碼 `0.3`、`2.5`、`60` 等數值。需加入帶有 `Rationale` 和 `Source` 的 `ParameterMetadata[T]`。參見 `docs/PARAMETER_SYSTEM.md`。範例：`FallbackPriceTargets` map（per-skill target/stop-loss 乘數，於 `monitoring/service/session.go` 透過 `config.GetParametersConfig()` 讀取）。 |
