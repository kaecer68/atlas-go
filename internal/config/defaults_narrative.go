...
func defaultIndustryParameters() IndustryParameters {
	return IndustryParameters{
		...
		ClassificationTree: ParameterMetadata[ClassificationTreeConfig]{
			Value: ClassificationTreeConfig{
				Segments: []IndustrySegmentConfig{
					...
					{ID: "leo_satellite", Name: "低軌衛星", NameEN: "LEO Satellite", Level: 1, Weight: 0.02, GeographicExposure: "Export", Cyclicality: "Cyclical", TechnologyIntensity: "HighTech", CapitalIntensity: "HighCapital", RepresentativeStocks: []string{"6271.TW", "2314.TW"}, Description: "衛星通訊新興供應鏈"},
					...
					{ID: "laser_communication", Name: "雷射通訊", NameEN: "Laser Communication", Level: 2, ParentID: "leo_satellite", Weight: 0.15, GeographicExposure: "Export", Cyclicality: "Cyclical", TechnologyIntensity: "HighTech", CapitalIntensity: "HighCapital", RepresentativeStocks: []string{"4979.TW", "6533.TW", "3163.TW"}, Description: "Optical inter-satellite links"},
				},
			},
		...
		SubstrateFromSymbolIndustryEnabled: ParameterMetadata[bool]{
			Value: true,
			Rationale: "Per-stock industry substrate gate (2026-09-24, issue #1943). ENABLED 2026-09-25 " +
				"by owner sign-off (issue #1971, plan A) after a read-only pre-flight showed the pre-gate " +
				"mapping resolved 0/10 sampled production symbols and universe_snapshot.json reported " +
				"symbols_ranked=0: the hard-coded representative-stock tables reached only ~27 symbols " +
				"(~3.2% of the listed universe), so every industry-level statistic (sector exposure, " +
				"SmartUniverse population, coverage audit) rested on a population too small to be " +
				"significant. The first-party symbol_industry channel (TWSE t187ap03_L 產業別 + TPEx " +
				"mopsfin_t187ap03_O SecuritiesIndustryCode, mapped through sectormap namespace " +
				"twse_industry_code) now supplies the per-stock field for the whole listed market: " +
				"1988 upstream symbols, 1599 mapped, 379 unmapped codes reported with reasons (never " +
				"guessed), 10 unknown (TDR code 91), 20/20 canonical L1 reachable. Rollback: set this " +
				"back to false and restart the atlas process — the gate is read at composition-root " +
				"wiring time, so a hot config reload is not sufficient.",
			Source: SourceExperimental,
			Todo:   "Observation window started 2026-09-25 (issue #1971). Promote to SourceHeuristic after >=20 sessions with zero invariant violations (symbol_industry channel ok and fresh, population and L1 coverage as expected, symbols_ranked > 0, sector weights non-zero and summing to 1, stockpicker still recommends, pre-existing channels unaffected, error/latency normal). Any violation => roll back to false and restart.",
		},
	}
}
