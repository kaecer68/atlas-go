...
func TestRotationSignal_IsValid(t *testing.T) {
	...
	if nilSig.IsValid() { t.Error("nil signal should be invalid") }
	if (&RotationSignal{Diff: 0}).IsValid() { t.Error("zero diff invalid") }
	if !(&RotationSignal{Diff: 0.01}).IsValid() { t.Error("positive diff valid") }
}

func TestStyleRotationStrategy_Basic(t *testing.T) {
	s := NewStyleRotationStrategy()
	if s.GetDetector() == nil { t.Error("GetDetector nil") }
	if s.GetAllocator() == nil { t.Error("GetAllocator nil") }
	result := s.UpdateAndRotate(map[Style]float64{StyleGrowth: 0.01, StyleValue: 0.01})
	if result != nil { t.Error("should be nil when equal") }
	stats := s.GetStats()
	if stats.CurrentLeader == "" { t.Error("leader empty") }
}
...
func TestCalculateStyleExposures(t *testing.T) {
	...
	if len(exp.Current) == 0 { t.Error("expected non-empty") }
}
...
func TestRotationSignal_AllFields(t *testing.T) {
	...
	if sig.FromStyle != StyleValue || sig.ToStyle != StyleGrowth { t.Error("fields mismatch") }
}
