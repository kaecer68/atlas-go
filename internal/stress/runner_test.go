...
func TestCovarianceDrivenScenarios(t *testing.T) {
	runner := NewRunner(domain.AgentRegistry{}, domain.ExecutionPolicy{})
	// 2×2 covariance: strong positive correlation
	cov := [][]float64{
		{0.04, 0.035},
		{0.035, 0.04},
	}
	...
	runner.SetPortfolioWeights(map[string]float64{"2330.TW": 0.6, "0050.TW": 0.4})
	...
	if result.MaxDrawdown < 0.05 {
		t.Errorf("cov-driven COVID crash: MDD = %.2f%%, expected > 5%%", result.MaxDrawdown*100)
	}
	if len(result.DailyValues) < 2 {
		t.Error("cov-driven: expected daily values path")
	}
	t.Logf("cov-driven COVID crash: MDD=%.2f%%, VaR95=%.4f, Sharpe=%.3f",
