...
func (s *System) RunDailySimulation(asOf time.Time) (domain.SimulationResult, error) {
	...
	outcomes := buildReplayOutcomes(outcomeRawRecs, outcomeFinalRecs, quotes, asOf, s.Sim().replay, regime)
	if len(outcomes) == 0 {
...
func (s *System) runReplaySimulation(sessionDate time.Time) (domain.SimulationResult, error) {
	...
	tw.Record(7, "ledger_write", "START", nil)
	outcomes := buildReplayOutcomes(outcomeRawRecs, outcomeFinalRecs, quotes, sessionDate, s.Sim().replay, regime)
	if s.Risk().repo != nil {
		_ = s.Risk().repo.RecordOutcomes(s.Sim().ctx, outcomes)
	}
	_ = s.Sim().ledger.RecordOutcomes(outcomes)
...
func buildSyntheticOutcomes(rawRecs, finalRecs []domain.Recommendation, quotes []domain.Quote, asOf time.Time, regime domain.Regime) []domain.RecommendationOutcome {
	if len(rawRecs) == 0 {
		return nil
	}
...
func buildReplayOutcomes(rawRecs, finalRecs []domain.Recommendation, quotes []domain.Quote, asOf time.Time, ds *replay.Dataset, regime domain.Regime) []domain.RecommendationOutcome {
