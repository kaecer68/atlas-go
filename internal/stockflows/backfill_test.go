...
func readFlowFileAt(t *testing.T, dir, symbol string) (stockpicker.FlowFile, error) {
	t.Helper()
	data, err := os.ReadFile(filepath.Join(dir, symbol+".json"))
	if err != nil {
		return stockpicker.FlowFile{}, err
	}
...
func readFlowFile(t *testing.T, path string) stockpicker.FlowFile {
	t.Helper()
