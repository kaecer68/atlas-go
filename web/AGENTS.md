...
**禁止手動編輯這兩個檔案（任何一份 copy）。** 任何手動修改都會在下次 `go generate ./...` 被覆寫,並讓 `quality.yml` 的 `generate` job CI 失敗。
...
2. **跑 `go generate ./...` 重新產出**全部 4 份 copy
3. **不要**直接編輯任何一個目錄下的 `field_types.ts` 或 `valid_fields.json`

### 若 commit 時預先擋住你

`.githooks/pre-commit` 的 Phase 5 會自動跑 `go generate ./...`,若**任一 copy** 有 drift 會**直接阻擋 commit** 並提示此訊息。這是設計行為,不是 bug。

若看到擋住訊息:

```text
❌ Pre-commit Phase 5: <dir>/static/js/shared/field_types.ts drift detected
...
正確修法:**不要**編輯 `field_types.ts` 或 `valid_fields.json`,而是回去修改對應的 Go struct(在 `internal/<pkg>/`)。
