...
function renderCompact() {
  ...
  let agentSummaryHtml = top5.map(function(a, i) {
    ...
    let sharpeColor = (a.sharpe || 0) > 1 ? 'let(--up)' : ((a.sharpe || 0) < 0 ? 'let(--down)' : 'let(--warn)');
    return '<div style="display:flex;align-items:center;gap:8px;padding:4px 0;border-bottom:1px solid let(--border)">' +
      '<span style="font-size:11px;color:let(--muted);width:18px">' + (i + 1) + '</span>' +
      '<span style="flex:1;font-size:12px">' + agentName(a.agent_id) + '</span>' +
      '<div style="width:80px;height:4px;background:let(--bg);border-radius:2px">' +
        '<div style="width:' + barW + '%;height:100%;background:' + (barW > 50 ? 'let(--up)' : 'let(--warn)') + ';border-radius:2px"></div>' +
      '</div>' +
      '<span style="font-size:11px;color:let(--muted);width:35px;text-align:right">' + ((a.hit_rate || 0) * 100).toFixed(0) + '%</span>' +
  ...
  let expHtml = '<div style="display:flex;gap:24px;margin-top:4px">' +
    '<div style="text-align:center"><div style="font-size:20px;font-weight:700;color:' + (judges.length > 0 ? 'let(--warn)' : 'let(--muted)') + '">' + judges.length + '</div><div style="font-size:11px;color:let(--muted)">待評判</div></div>' +
    '<div style="text-align:center"><div style="font-size:20px;font-weight:700;color:' + (promotes.length > 0 ? 'let(--up)' : 'let(--muted)') + '">' + promotes.length + '</div><div style="font-size:11px;color:let(--muted)">待晉升</div></div>' +
    '<div style="text-align:center"><div style="font-size:20px;font-weight:700">' + scorecards.length + '</div><div style="font-size:11px;color:let(--muted)">活躍 Agent</div></div>' +
    '</div>';
  ...
  el.innerHTML = '<div class="panel wide" style="margin-bottom:12px;padding:10px 14px">' +
    ...
    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">' +
      ...
      '<div style="font-size:11px;color:let(--muted)">' + sessions.length + ' 個 session</div>' +
    '</div>' +
    regimeHtml +
    '</div>' +
  ...
  setTimeout(function() { renderInlineEquityCurve('inlineEquityCurve', curvePoints); }, 100);
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">' +
      ...
      '<div class="panel" style="padding:10px 14px">' +
        ...
        expHtml +
        '<div style="margin-top:12px;font-size:11px;color:let(--muted)">' +
...
function renderDetailed() {
  ...
  let scoreboardHtml = '<table style="width:100%;font-size:12px;border-collapse:collapse">' +
    '<thead><tr style="border-bottom:2px solid let(--border)">' +
  ...
  for (let i = 0; i < sorted.length; i++) {
    let a = sorted[i];
    let sharpeColor = (a.sharpe || 0) > 1 ? 'let(--up)' : ((a.sharpe || 0) < 0 ? 'let(--down)' : 'let(--warn)');
    let hitColor = (a.hit_rate || 0) > 0.5 ? 'let(--up)' : ((a.hit_rate || 0) > 0.3 ? 'let(--warn)' : 'let(--muted)');
    scoreboardHtml += '<tr style="border-bottom:1px solid let(--border)">' +
      '<td style="padding:6px">' + agentName(a.agent_id) + '</td>' +
      '<td style="padding:6px;color:let(--muted)">' + (a.skill || '-') + '</td>' +
      ...
      '<td style="padding:6px;text-align:right;color:let(--down)">' + ((a.max_drawdown || 0) * 100).toFixed(1) + '%</td>' +
      '</tr>';
  }
  ...
  if (allExps.length > 0) {
    ...
    for (let j = 0; j < allExps.length; j++) {
      let e = allExps[j];
      let statusBadge = e.status === 'running' ? '<span style="color:let(--warn)">● 進行中</span>' :
        (e.status === 'completed' ? '<span style="color:let(--up)">✓ 完成</span>' : '<span style="color:let(--muted)">○ ' + e.status + '</span>');
      expLogHtml += '<div style="padding:6px 0;border-bottom:1px solid let(--border)">' +
        '<div><strong>' + agentName(e.target_agent_id) + '</strong> · ' + (e.mutation_type || '實驗') + '</div>' +
        '<div style="color:let(--muted);font-size:11px">' + (e.mutation_summary || '') + '</div>' +
    ...
    expLogHtml = '<div style="padding:20px;text-align:center;color:let(--muted)">當前無實驗記錄。系統處於穩態運行。</div>';
  }
  ...
  if (tab === 'agents') {
    ...
    for (let l = 0; l < layers.length; l++) {
      ...
      for (let j = 0; j < agents.length; j++) {
        ...
        let barColor = barLen > 60 ? 'let(--up)' : (barLen > 30 ? 'let(--warn)' : 'let(--muted)');
        html += '<div style="display:flex;align-items:center;gap:8px;padding:3px 0">' +
          '<span style="font-size:11px;flex:1">' + agentName(a.agent_id) + '</span>' +
          '<span style="font-size:10px;color:let(--muted);width:40px">命中</span>' +
          '<div style="width:100px;height:5px;background:let(--bg);border-radius:3px">' +
            '<div style="width:' + barLen + '%;height:100%;background:' + barColor + ';border-radius:3px"></div>' +
          '</div>' +
          '<span style="font-size:10px;color:let(--muted);width:30px;text-align:right">' + ((a.hit_rate || 0) * 100).toFixed(0) + '%</span>' +
          '<span style="font-size:10px;color:let(--muted);width:55px;text-align:right">S:' + (a.sharpe || 0).toFixed(2) + '</span>' +
          '</div>';
      }
      html += '</div>';
    }
    el.innerHTML = html || '<div style="padding:20px;color:let(--muted);text-align:center">無 Agent 數據</div>';
  }
  else if (tab === 'regime') {
    el.innerHTML = '<div style="font-size:13px;font-weight:700;margin-bottom:8px">Regime 演化時間線</div>' +
      '<div style="font-size:11px;color:let(--muted);margin-bottom:12px">共 ' + sessions.length + ' 個 session</div>' +
    ...
    if (all.length > 0) {
      for (let i = 0; i < all.length; i++) {
        ...
        html += '<div style="padding:8px 0;border-bottom:1px solid let(--border)">' +
          '<div style="display:flex;justify-content:space-between;align-items:center">' +
            '<div><strong>' + agentName(e.target_agent_id) + '</strong> · ' + (e.mutation_type || '') + '</div>' +
            '<span style="font-size:11px;color:' + (isJudge ? 'let(--warn)' : 'let(--up)') + '">' + (isJudge ? '待評判' : '待晉升') + '</span>' +
          '</div>' +
          '<div style="font-size:11px;color:let(--muted);margin-top:2px">' + (e.mutation_summary || '') + '</div>' +
      ...
      html = '<div style="padding:20px;color:let(--muted);text-align:center">無實驗記錄</div>';
    }
    el.innerHTML = html;
  }
}
...
function renderRegimeTimeline(sessions, maxBars) {
  if (!sessions || !sessions.length) return '<div style="padding:8px;color:let(--muted);font-size:11px">無 regime 歷史數據</div>';
  ...
  let colors = { RISK_ON: 'let(--up)', RISK_OFF: 'let(--down)', NEUTRAL: 'let(--warn)' };
  let segments = [];
  ...
  for (let s = 0; s < segments.length; s++) {
    ...
    let color = colors[seg.regime] || 'let(--muted)';
    html += '<div title="' + regimeLabel(seg.regime) + ': ' + seg.count + ' sessions" ' +
      ...
      (seg.count > 3 ? '<span style="position:absolute;left:4px;top:50%;transform:translateY(-50%);font-size:9px;font-weight:600;color:let(--bg);white-space:nowrap">' + regimeLabel(seg.regime) + '</span>' : '') +
      '</div>';
  }
  html += '</div>';

  html += '<div style="display:flex;gap:16px;font-size:10px;color:let(--muted)">' +
    '<span style="color:let(--up)">🟢 多頭</span>' +
    '<span style="color:let(--down)">🔴 空頭</span>' +
    '<span style="color:let(--warn)">🟡 盤整</span>' +
