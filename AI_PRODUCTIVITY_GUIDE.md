...
## 11. EXECUTION KICKOFF PACK (2026-04-09)

This section is the direct launch plan for implementation, designed for action without further clarification.

### 11.1 Master Index Map (M0-M8)

| ID | Objective | Output | Acceptance Gate |
|----|-----------|--------|-----------------|
| M0 | Governance setup | ADR + ownership + change protocol | Decision flow is documented and signed off |
| M1 | Data contract freeze | event/factor/proposal/commit JSON schemas | Contract tests pass |
| M2 | Macro Intelligence lane | event ingestion + factor snapshots | Replay generates stable factor timeline |
| M3 | Decision trace lane | proposal -> commit -> approve -> execute trace | Every execution has full trace IDs |
| M4 | Guard pipeline v2 | pre-trade and post-trade control chain | Blocking reasons are auditable |
| M5 | Parallel simulation | scenario runner and baseline compare | Same input yields deterministic output |
| M6 | Visualization baseline | macro radar + agent observatory + forecast-vs-reality | Dashboard reads real trace data |
| M7 | Human-in-the-loop | approve/reject workflow and rollback | One-click revert succeeds |
| M8 | Production readiness | SLO + alerting + runbook | Staging drills pass |

### 11.2 Implementation Tasks (T-001 to T-005)

| Task | Scope | Main Files | Done Definition |
|------|-------|------------|-----------------|
| T-001 | Decision state machine | `internal/experiment/`, `internal/evolution/` | Invalid transitions are rejected with explicit error |
| T-002 | Proposal/commit schema | `internal/domain/`, `cmd/execute-experiment/` | Schema validation blocks malformed records |
| T-003 | Guard pipeline v2 | `internal/orchestrator/`, `internal/sim/` | Soft/hard guard result is persisted in ledger |
| T-004 | Trace persistence | `internal/ledger/` | Proposal ID and commit ID available in every session summary |
| T-005 | Dashboard API foundation | `cmd/atlas/`, `internal/monitoring/` | Macro/agent/forecast endpoints return contract-compliant JSON |

### 11.3 Daily Execution Rhythm

1. 09:00-09:20: Read previous run artifacts from `data/state/sessions/`.
2. 09:20-10:30: Complete one atomic implementation slice from T-001 to T-005.
3. 10:30-11:00: Run focused tests on impacted packages.
4. 14:00-15:30: Integrate with replay and validate trace completeness.
5. 15:30-16:00: Run gate checklist and write decision note.

### 11.4 Mandatory Command Sequence
...
# 2) experiment path integrity
go test ./internal/experiment/...
go test ./internal/evolution/...

# 3) full confidence before merge
go build ./...
go test ./...
```

### 11.5 Release Gates (Do Not Skip)

- Gate G1: Contract gate (schemas + compatibility checks pass)
- Gate G2: Behavior gate (replay determinism preserved)
- Gate G3: Safety gate (guard pipeline blocks high-risk actions)
- Gate G4: Audit gate (trace IDs and reasons fully recoverable)
- Gate G5: Operations gate (runbook and rollback verified)

### 11.6 First Implementation Order

1. T-002 proposal/commit schema
2. T-001 state machine enforcement
3. T-004 trace persistence
4. T-003 guard pipeline v2
5. T-005 dashboard API baseline

This order minimizes rework by locking contracts before behavior and API exposure.
