...
window._paramMapExpand = function(span) {
  ...
  for (const [k, v] of entries) {
    ...
    html += `<tr>
      ...
      <td class="param-val-editable" title="點擊編輯" data-key="${escapeHtml(subKey)}" data-val="${escapeHtml(String(v))}" onclick="window._paramEdit(this)">${escapeHtml(typeof v === 'number' ? fmtSafeNumber(v, { decimals: 4 }) : String(v))}</td>
    </tr>`;
  }
  html += '</tbody></table>';
