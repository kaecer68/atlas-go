...
| **前端欄位命名不一致** | 前端 JavaScript 必須使用 **snake_case** 存取 API 回應欄位（如 `g.input_count`），不可使用 PascalCase（如 `g.InputCount`）。JavaScript 存取不存在的屬性會靜默回傳 `undefined`，不會報錯。→ **已透過 `cmd/gentags` 自動生成 `field_names.js` 解決**：從 Go struct JSON tag 自動同步到前端類型定義，`FIELD.GuardOutcome.input_count` 編寫時即獲 LSP 提示。 |
| **前端代碼模組化** | `web/static/index.html` 已從 ~4000 行單體改為 ~1000 行 HTML + 13 個 JS 模組。頁面邏輯在 `js/pages/*.js`，共用工具在 `js/shared/*.js`，入口在 `js/main.js`。**禁止在 index.html 中新增內聯 `<script>` 塊。新功能一律新增模組檔案。** |
| **Go → JS 類型自動生成** | 執行 `go run ./cmd/gentags` 從 `internal/domain/*.go` 自動生成 `web/static/js/shared/field_names.js`（48 個 struct 的所有欄位名）。後端新增欄位後必須重跑此命令以保持同步。
