...
> **本文件**：給 AI agent 看的「何時該呼叫哪個 tool」決策表 + 完整 catalog（約 84 個 tool，實際數量依 MCP server config 而定：基礎 82 個，`SamplingEnabled` / `ElicitationEnabled` 啟用最多 +2）。確切數字由 `mcp/tools/list` 或 `system_get_health` 回傳。
...
## 完整工具 Catalog（約 84 個 tool，Phase 2.2 全部上線）
