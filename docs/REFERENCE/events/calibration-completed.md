...
| 既有組件 | `shared_web/static/js/components/risk-gate-panel.js` | 操作面板，**非 SSE-driven** |
| 即時訂閱 | `admin_web/static/js/event-listeners.js` | 透過 `eventSource.on('experiment.calibration_completed', handler)` 訂閱 |
