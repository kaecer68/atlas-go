...
| 找不到 `prism_get_training_results` tool | **預期**（#1527，2026-08-12）— PRISM 未啟用，MCP 工具與 `/api/prism/training-results` 已移除；`scheduler_get_status` 中 `prism_training` disabled 亦為預期。詳見 `.omo/audit/ATLAS_SYSTEM_STATE.md` §重大事件摘要 8 |
...
- **117 個 tool 總覽**（sampling/elicitation 全開 119）: [`docs/reference/tool-catalog.md`](./reference/tool-catalog.md)
