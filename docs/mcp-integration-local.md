...
| 找不到 `prism_get_training_results` tool | **預期**（#1527，2026-08-12）— PRISM 未啟用，MCP 工具與 `/api/prism/training-results` 已移除；`scheduler_get_status` 中 `prism_training` disabled 亦為預期。 |
