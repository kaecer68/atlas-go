...
> - L2.3 PoC(`SemiconductorLLMAgent` + `DriverAdapter`)是本框架的**第一個 sector agent consumer**,驗證了 end-to-end flow。詳見 [`docs/wave-11/L2_3_PLAN_REFLECT.md`](wave-11/L2_3_PLAN_REFLECT.md)。
...
- [六、Provider 路由策略（Provider Routing Strategy）](#六provider-路由策略provider-routing-strategy)
- [七、資料流與審計軌跡（Data Flow & Audit Trail）](#七資料流與審計軌跡data-flow--audit-trail)
- [八、遷移路徑（Migration Roadmap）](#八遷移路徑migration-roadmap)
- [九、風險與緩解（Risks & Mitigations）](#九風險與緩解risks--mitigations)
- [十、決策紀錄（Decision Log）](#十決策紀錄decision-log)
...
// Capability 是一個語意穩定的字串 ID，定義「呼叫端想要什麼」。
// 新增 capability 必須：
//   1. 在 docs/llm-integration-strategy-framework.md §三的能力分類學中登記
//   2. 對應到一個 CapabilityRequest 型別（見 §4.3）
//   3. 在 CapabilityRegistry 註冊至少一個 provider 實作
type Capability string

const (
    CapabilityFailureAttribution      Capability = "strategy.failure_attribution"
    CapabilityRationaleTranslation    Capability = "narrative.rationale_translation_fallback"
    CapabilityStrategyFrameSummary    Capability = "strategy.frame_summary"
    CapabilityPromptLint              Capability = "dev.prompt_lint"
    CapabilityPRISMCohortInsight      Capability = "orchestrator.prism_cohort_insight"
    CapabilityGapDescription          Capability = "spawning.gap_description_enrichment"
    CapabilityNarrativeEventHeadline  Capability = "narrative.event_headline"
    CapabilityConfidenceCommentary     Capability = "risk.confidence_calibration_commentary"
    CapabilityCodeReviewAnnotation    Capability = "dev.code_review_annotation" // v2.0 新增
)

// Provider 是 provider 的語意 ID。
// 同一 capability 必須可由多個 provider 提供；路由器依 §3.2 決策表 + 健康度選擇。
type Provider string

// v2.0 Provider 常數：補齊三主力 + 兩個備援；OpenAI 標為 DEPRECATED（用戶不使用）。
const (
    ProviderKimi        Provider = "kimi"         // kimi.com coding plan：K2.7 (code)、K2.6 (general)
    ProviderMiniMax     Provider = "minimax"      // minimax.com coding plan：M3 general multimodal
    ProviderDeepSeek    Provider = "deepseek"     // DeepSeek 官方平台：V4-Pro / V4-Flash
    ProviderOpenCodeGo  Provider = "opencode-go"  // 多模型訂閱服務（主要備援）
    ProviderOpenCodeZen Provider = "opencode-zen" // 備援平台（次要備援）
    ProviderMock        Provider = "mock"         // 測試樁
    // DEPRECATED：v1.0 預留，v2.0 確認用戶不使用 OpenAI；保留常數以避免破壞既有 config。
    // ProviderOpenAI Provider = "openai"
)

// Request 是 capability 呼叫的統一信封。任何 capability 都從這個 envelope
// 開始，再 unmarshal 到自己的強型別 payload。
type Request struct {
    Capability Capability
    Payload    any     // capability-specific 強型別，由 capability 註冊時約定
    Options    Options
    // v2.0 新增：若呼叫端已知資料分類（regulated / non-regulated），
    // 可在此欄位提示路由器走「資料主權閘門」（見 ADR-010）。
    DataClass  DataClass
}
...
const (
    DataClassUnmarked     DataClass = "unmarked"     // 未分類；路由器依 capability 預設
    DataClassNonRegulated DataClass = "non_regulated" // 一般資料
    DataClassRegulated    DataClass = "regulated"    // 受規範金融資料；禁止走 MiniMax hosted
    DataClassSecret       DataClass = "secret"       // 含營業秘密；只能走自架
)

// Options 控制單次呼叫的非功能參數。呼叫端可覆寫，路由器可強制覆寫。
type Options struct {
    MaxTokens       int
    Temperature     float64
    TimeoutMillis   int
    CacheTTLSeconds int
    Trace           bool
    // v2.0 新增：Provider 顯式覆寫（測試或 sticky routing 用）。
...
type Response struct {
    Capability Capability
    Provider   Provider   // 實際執行此呼叫的 provider（可能不同於 capability 預設的 primary）
    Output     any        // capability-specific 強型別
    Usage      Usage
    TraceID    string
    CacheHit   bool
    // v2.0 新增：fallback 鏈軌跡。
    AttemptedProviders []Provider `json:"attempted_providers,omitempty"`
}

// Usage 是統一 token 帳戶。所有 provider 必須回報同一格式。
type Usage struct {
    PromptTokens     int64
...
// Provider 是 capability 的執行者。一個 Provider 必須對**它支援的 capability**
// 實作相對應的 *Handler 方法；不支援的 capability 由路由器 fallback。
type Provider interface {
    // Name 回傳 provider ID（用於 metrics label、log、trace）。
    Name() Provider

    // Supports 回傳此 provider 對給定 capability 是否有實作。
    Supports(Capability) bool

    // Call 執行單次 capability 呼叫。Provider 必須負責：
    //   - 速率限制（rate limiter）
    //   - circuit breaker
    //   - 自動 retry（僅 5xx / 429）
    //   - 結構化 metric 與 audit record
    Call(ctx context.Context, req Request) (Response, error)
}

// Router 是呼叫端持有的介面。所有業務程式碼只跟 Router 對話。
type Router interface {
    // Call 依 capability 路由到合適 provider，並回傳結果。
    // 若首選 provider 失敗，路由器依 Policy 決定 fallback（見 §6.3）。
    Call(ctx context.Context, req Request) (Response, error)

    // Health 回傳所有 provider 的當前健康狀態（給 monitoring endpoint）。
    Health() RouterHealth
}

// RouterHealth 是 /healthz/llm 端點的回傳格式。
type RouterHealth struct {
    Providers     map[Provider]ProviderHealth `json:"providers"`
    Capabilities  map[Capability][]Provider  `json:"capabilities"`
    // v2.0 新增：四級路由鏈對應的 provider 列表。
    RoutingTable  map[Capability]RoutingChain `json:"routing_table,omitempty"`
}
...
// RoutingChain 是 v2.0 新增：每個 capability 對應的完整 fallback 鏈。
type RoutingChain struct {
    Primary    Provider `json:"primary"`
    Backup1    Provider `json:"backup1"`
    Backup2    Provider `json:"backup2"`
    LastResort string   `json:"last_resort"` // 描述非 provider 行為，如 "rule_based" / "passthrough" / "discard"
}
```

### 4.3 CapabilityRequest 強型別範例

> 每個 capability 在 router 內部有一個專屬 handler function；對外公開的是強型別 payload。

```go
// internal/llm/capabilities/failure_attribution.go

type FailureAttributionRequest struct {
    FailureContext llm_annotator.FailureContext // 重用現有型別
    DataClass      llm.DataClass                // v2.0 新增：regulatory hint
}

type FailureAttributionResponse struct {
    Explanation string  `json:"explanation"`
    Confidence  float64 `json:"confidence"` // v2.0 新增：回報 confidence
}

// HandlerSignature: func(ctx, FailureAttributionRequest, Options) (FailureAttributionResponse, Usage, error)
```

```go
// internal/llm/capabilities/rationale_translation.go

type RationaleTranslationRequest struct {
    English    string
    CorpusMiss bool   // 必須由呼叫端確認 corpus miss 才允許送 LLM
    Domain     string // "sector" | "style" | "superinvestor" | "control"
}

type RationaleTranslationResponse struct {
    Chinese      string
    Confidence   float64 // 0..1；呼叫端可用於 gate 是否採用
}
```

```go
// internal/llm/capabilities/code_review_annotation.go（v2.0 新增）

type CodeReviewAnnotationRequest struct {
    Diff         string // 完整 diff
    FileLanguage string // "go" | "python" | "ts" | "..."
    ContextLines int    // 上下文行數
}
...
type CodeReviewAnnotation struct {
    Line       int    `json:"line"`
...
> **不改 `Annotator`**（依原則 1）；用 adapter pattern 包進去。

```go
// internal/llm/adapters/annotator_adapter.go（提案）

// annotatorAdapter 把既有 Annotator（介面合約為
// internal/llm_annotator/doc.go:75-80）包成新的 Provider。
type annotatorAdapter struct {
    inner   llm_annotator.Annotator
    metrics llm.MetricsRecorder
    store   llm.RecordStore
    // v2.0 新增：底層 model 標籤；K2.7 與 K2.6 用同一 endpoint 但 model id 不同。
    modelID string // "kimi-k2.7" 或 "kimi-k2.6"
}

func (a *annotatorAdapter) Name() llm.Provider { return llm.ProviderKimi }

func (a *annotatorAdapter) Supports(c llm.Capability) bool {
    switch c {
    case llm.CapabilityFailureAttribution:
        return a.modelID == "kimi-k2.6" // K2.6 為 general instruct；K2.7 不可用於此 capability
    case llm.CapabilityCodeReviewAnnotation:
        return a.modelID == "kimi-k2.7" // K2.7 為 code model
    default:
        return false
    }
}
...
func (a *annotatorAdapter) Call(ctx context.Context, req llm.Request) (llm.Response, error) {
    ...
    if !ok {
        return llm.Response{}, fmt.Errorf("failure_attribution: payload must be FailureContext, got %T", req.Payload)
    }
    text, err := a.inner.Annotate(ctx, fc)
    if err != nil {
    ...
    return llm.Response{
        ...
        Provider:   a.Name(),
        Output:     FailureAttributionResponse{Explanation: text},
        // Usage 由內部 Annotator 統計；adapter 從最近一次呼叫回填
        Usage: a.inner.Usage(), // 既有 KimiClient.Usage() 已存在（annotator.go:184-188）
    }, nil
}
```

> **關鍵設計點**：`Annotator` 介面（`doc.go:75-80`）完全不動；adapter 把 `KimiClient.Usage()`、`Snapshot()`、`RecentAnnotations()` 等所有既有觀測 API 映射成新介面的對等概念。新介面**吸收**而不是**取代**。
>
> v2.0 補充：Kimi provider 內部依 `modelID` 區分 K2.6 / K2.7；K2.7 的 `Supports` 對 `CapabilityFailureAttribution` 回傳 `false`，確保「純程式碼模型用於敘事任務」這條規則在程式碼層就有 guard（不只是 ADR 文件約束）。

### 4.5 既有模組如何改寫呼叫端

#### 4.5.1 monitoring/api/strategies/handlers.go（最小幅度）
...
改成：
```go
// handlers.go:302（提案；同位置最小幅度改寫）
resp, err := h.router.Call(r.Context(), llm.Request{
    Capability: llm.CapabilityFailureAttribution,
    Payload:    fc,
    DataClass:  llm.DataClassRegulated, // v2.0 新增：歸因含策略資料，標為 regulated
})
if err != nil {
    // 既有 fallback 行為完全保留：handlers.go:303-309
    return http.StatusBadGateway, map[string]any{
        "error":    err.Error(),
        "fallback": frame.Attribution,
        "backend":  h.annotator.Name(),
        "attempted_providers": resp.AttemptedProviders, // v2.0 新增
    }
}
text := resp.Output.(FailureAttributionResponse).Explanation
confidence := resp.Output.(FailureAttributionResponse).Confidence
```

`h.annotator` 仍保留為 `Handlers.annotator` 結構欄位（`handlers.go:20`），新介面 `h.router` 並存。**完全不需要改 main.go 的 wiring**（`main.go:1613-1624` 仍可繼續呼叫 `SetStrategiesAnnotator(kimi)`）。

#### 4.5.2 narrative/rationale_corpus.go（無侵入式擴張）

> 本檔的 invariant（`rationale_corpus.go:15-21`）必須保留：「**MUST NOT depend on an LLM**」。

解法：**corpus 本身不變**；把 LLM fallback 放到呼叫端而非 corpus 內。具體位置是 `internal/monitoring/api/pipeline/handlers.go`（corpus 的實際呼叫點之一，依 `rationale_corpus.go:6-13` 註解），在 step 6 passthrough（`rationale_corpus.go:208-209`）發生時，handler 改為：

```go
// 偽碼（位於 pipeline/handlers.go，**非 rationale_corpus.go**）
translated := narrative.TranslateReason(en)
if isPassthrough(en, translated) { // 表示 corpus 沒命中
    if h.router != nil {
        resp, err := h.router.Call(ctx, llm.Request{
            Capability: llm.CapabilityRationaleTranslation,
            Payload:    RationaleTranslationRequest{English: en, CorpusMiss: true},
            DataClass:  llm.DataClassNonRegulated, // rationale 文字不含敏感欄位
        })
        if err == nil && resp.Output.(RationaleTranslationResponse).Confidence > 0.7 {
            translated = resp.Output.(RationaleTranslationResponse).Chinese
        }
        // 否則仍 passthrough 原字，與今天一致
    }
}
```

**這保證了 `rationale_corpus.go` 仍然是純 Go map**，對其 invariant 零影響。v2.0 補充：Rationale Translation 的 primary 改為 DeepSeek V4-Flash（最低成本），這條決策直接降低 corpus miss 時的營運成本。

#### 4.5.3 spawning/agent_factory.go（透過 GapDetector hook）

`agent_factory.go:33-78` 的 `CreateAgentForGap` 目前**沒有 LLM 呼叫**。新做法：

1. `gap_detector.go` 在偵測到 gap（`:17-28` 的 `KnowledgeGap`）後，把 `Description` 欄位送 Router（capability `GapDescription`）。
2. 回傳值寫入 `KnowledgeGap.Description` 增強欄位（新增 `EnrichedDescription string` 欄位，**不覆蓋**原 `Description`，避免行為變化）。
3. `agent_factory.go:71` 的 `OperatingNotes` 可選擇性附加 enrichment 結果（仍由 caller 決定）。
4. v2.0 新增：enrichment 失敗時 `DataClass` 應標為 `non_regulated`（gap description 不含帳號/統編等敏感欄位），允許降級到 OpenCode-Go 備援。

這樣 `spawning` 套件本身仍是純統計 + 字串生成（與 `gap_detector.go:1-13` 的設計哲學一致），LLM 在**外層事件消費端**完成。

#### 4.5.4 orchestrator/prism_executor.go（訓練結案 hook）

`prism_executor.go:30-103` 的 `Run` method 在回傳前（`:94-102`）新增一個**非阻塞、錯誤容忍**的 hook：

```go
// prism_executor.go:94 前（提案）
result := prism.TrainingResult{...}
if e.insightRouter != nil {
    go func() {
        ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
        defer cancel()
        _, _ = e.insightRouter.Call(ctx, llm.Request{
            Capability: llm.CapabilityPRISMCohortInsight,
            Payload: PRISMCohortInsightRequest{
                AgentID: task.AgentID,
                Regime:  task.Regime,
                Result:  result,
            },
            DataClass: llm.DataClassRegulated, // v2.0 新增：PRISM 結果含策略績效，標為 regulated
        })
        // 錯誤完全忽略；不影響 PRISM 訓練本身
    }()
}
return result, nil
```

注意：`e.insightRouter` 為**可選注入**（nullable）；`Run` 的既有 contract（`prism.TrainingResult`、error）完全不變。這是 S 級模組接受 X 級能力的標準模式——**靠介面隔離，不靠 import 隔離**。

v2.0 補充：因為 PRISM 結果是 regulated，PRISMCohortInsight 的 primary 從 v1.0 預設的 Kimi 改為 MiniMax M3（金融領域強），但因 M3 hosted 受 §9 風險 8 規範，**生產環境必須走自架 M3**。此一限制由 Router 的 `DataClassRegulated` 閘門強制執行（見 §6.1）。
...
## 六、Provider 路由策略（Provider Routing Strategy）
...
## 八、遷移路徑（Migration Roadmap）

> **硬約束**：任何階段都**不得破壞現有 production 行為**。每個階段結束時，現有 endpoint 行為與 metrics 必須 byte-for-byte 等價（除新增 capability / provider label 外）。
