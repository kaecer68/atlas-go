...
> **119 tools**（預設啟用；sampling/elicitation feature-gated 全開時 121）grouped by functional area. For investor use cases, see [`docs/investor/use-cases/`](../investor/use-cases/).
> For natural language query examples, see [`docs/investor/query-examples.md`](../investor/query-examples.md).

## 工具數量

業務 110+ + template_detector 2 + audit 4 + audit_state 1 + strategy_for_period 1 + stock_get_monthly_revenue 1 + stock_get_win_rate 1 + stock_picker_scan 1 + Phase 2 alert lifecycle 4 − prism_get_training_results 1（#1527 移除）= **119**（**基礎 119**, **+2** sampling/elicitation feature-gated 預設關閉 → 最多 121；啟動期 assert ∈ [117, 123]，見 `cmd/atlas-mcp/server/server.go`）
