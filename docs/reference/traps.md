...
| **`Auto-cycle-update error 不要只信 channel_health.json 的 record** | marketdata / industry | `/api/dashboard/channel-health` 上 `auto_cycle_update` channel 從某日起 `status=error` + `updated_at` 凍結，**不代表 task 真的卡住**。task 仍在 BackgroundTaskManager 排程（每 6h）持續跑，每次跑仍會失敗，但 channel_health.json record 不會被刷新（auto_cycle_update 不是 channel registry 的一員）。**正確診斷流程**：先用 MCP `task_get` 確認 `last_run` / `consecutive_failures` / `last_error`，再決定是不是 quota / 結構性問題。完整調查紀錄見 [`docs/investigations/2026-08-05-auto-cycle-update-quota-misconception.md`](../investigations/2026-08-05-auto-cycle-update-quota-misconception.md)。 |
