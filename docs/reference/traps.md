...
### Industry / Sector 命名空間（#1943 後續）[NEW — 2026-09-24]

| 陷阱 | 所屬模組 | 說明 |
|------|---------|------|
| **ETF 持股權重合計 ≠ 100%** | sectorallocation / sectormap | 投信官網持股頁只列**股票部位**，期貨／現金／保證金另計（2026-09-24 實測：0050 股票 99.71%、00878 97.33%、00929 96.59%、0056 98.36%）。不可把官網權重直接當 L1 向量，也不可用「補到 1」掩蓋；必須以**可對映持股權重**重新歸一，未覆蓋比例由 `ETFRepresentative.ReportedWeightPct - MappedWeightPct` 具名揭露（`internal/sectormap` 的 `Evidence()` 會把它寫進 mapping reason）。 |
| **TWSE 產業別是 2 位數字碼，中文名不在 OpenAPI 回應裡** | sectormap / marketdata | `opendata/t187ap03_L`（上市）的 `產業別` 與 TPEx `mopsfin_t187ap03_O`（上櫃）的 `SecuritiesIndustryCode` 只回數字碼。中文名要取 TWSE ISIN 對照表（`https://isin.twse.com.tw/isin/class_i.jsp?kind=1`；網頁為 **UTF-8 但宣告 ms950**，強制 Big5 解碼會得到亂碼）。**該對照表與 MOPS 舊碼表不同**：此處 `14` = 建材營造業（半導體業是 `24`，`25` = 電腦及週邊設備業）——套錯碼表會把整個電子業對到營造業。驗證方式：用已知個股交叉檢查（2330→24、2603→15、2912→18、2059→28）。完整碼表在 `internal/sectormap/twse_industry_code.go`。 |
| **同一個 TWSE 產業的兩種詞彙必須給出同一個 L1** | sectormap | 「半導體類指數」vs 產業碼 `24`、電腦及週邊設備類指數 vs `25`、電子零組件類指數 vs `28`、電器電纜類指數 vs `06`…共 22 組。兩張表若各自漂移，同一個 TWSE 產業會因資料來源不同而落到不同 canonical L1（正是 #1943 要消滅的缺陷）。`TestDrift_TWSECodeAndIndexNameAgree` 逐一比對這 22 組。 |
| **ETF 持股是快照，不是常態** | sectorallocation | ETF 每季調整成分股、持股權重每日變動。`internal/sectorallocation/testdata/etf_holdings_20260924.json` 是**具日期**的證據快照，`internal/sectormap` 的宣告表由它推導，測試每次重跑推導並逐值比對。更新持股 → 必須同時重跑推導更新宣告表，否則 `TestETFRepresentatives_DerivationMatchesDeclaredTable` 紅燈。**不要**把 ETF 期貨／槓桿部位算進 L1 曝險。 |
