...
| **WA-203** | C07 Sector Prediction Observation | `cmd/experimental/c07-day-evaluator` + `cmd/experimental/c07-spot-check-recorder` + `docs/operations/sector-prediction-runbook.md` + `docs/archive/sector-prediction-observation-log.md` | Day 7 / Day 14 evaluator 跑（每日 09:00 cron，見 runbook §3）+ operator 手動 spot-check | `sector-prediction-eval-day-7.md` / `sector-prediction-eval-day-14.md` report（位於 `docs/operations/`） | Day 14 promotion gate（待 7/30 跑） |
...
| **WA-801.1** | 資金流向歷史 | `internal/capitalflow/handler.go` | HTTP `GET /api/capital-flow/history?days=N` | 多日滾動樣本時序（max 252） | PR #1228 CL-1 新增；CF-INV-15/16/17 |
