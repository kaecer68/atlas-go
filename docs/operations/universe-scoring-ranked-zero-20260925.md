...
> **落地狀態（2026-09-25 更新，讀本節前先看這裡）**：本節的「示意」規則**已經不再是全部待辦**。
> - 本節草案 (1)(2) → **已落地**，但**改寫**（草案寫的 `_last` gauge **不存在**；改用
>   `increase()` 判定）：`monitoring/rules/atlas_universe_scoring_alerts.yml` 的第 1/3 條
>   （任務 M，PR #1985 已合併），加一條相鄰上游的「篩選全滅」= 第 2 條。
> - 缺口 (c)（`symbols_screened_total` 被改名/移除 ⇒ 三條同時沉默）與缺口 (d)-`empty_filtered`
