...
> **未來工作**: [`./l2-4-followup.md`](./l2-4-followup.md) — auto-cron / CLI flag / promotion procedure
...
- [ ] **Log 檔案建檔**: 見 `docs/operations/l2-4-observation-log.md` (Week 0 Baseline)
...
從 slog JSON output 拉出下列欄位,彙整至觀察記錄(見 `docs/operations/l2-4-observation-log.md`):
...
4. **記錄異常**: 在觀察記錄(見 `docs/operations/l2-4-observation-log.md`)標註 rollback 時間與觸發條件。
...
完整 4 步工作報告見 [`./l2-4-followup.md`](./l2-4-followup.md) §3。
...
| **觀察期 ownership 衝突** | 多 team 對 promotion gate 意見分歧 | 以觀察記錄(見 `docs/operations/l2-4-observation-log.md`)數據為準;由 issue owner 仲裁 | Kaecer(product owner) |
...
- 觀察期結束決策: 在觀察記錄(見 `docs/operations/l2-4-observation-log.md`)末段總結,並 link 到後續 promotion / rollback PR。
