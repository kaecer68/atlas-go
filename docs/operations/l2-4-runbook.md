...
從 slog JSON output 拉出下列欄位,彙整至觀察記錄(範本待後續 PR 提供):
...
4. **記錄異常**: 在觀察記錄(範本待後續 PR 提供)標註 rollback 時間與觸發條件。
...
| **觀察期 ownership 衝突** | 多 team 對 promotion gate 意見分歧 | 以觀察記錄(範本待後續 PR 提供)數據為準;由 issue owner 仲裁 | Kaecer(product owner) |
...
- 觀察期結束決策: 在觀察記錄(範本待後續 PR 提供)末段總結,並 link 到後續 promotion / rollback PR。
