...
|---|---|---|---|---|---|---|
| `start`（窗口開啟） | 28 | 216 | **0.3754 [0.2815, 0.4793]** | 0.4255 | 0.5067 [0.3126, 0.6347] | **已更正**（CI 上界 < 0.5、探針未偵測記憶、且通過 200 case 覆蓋閘門） |
| `peak`（事件高峰） | 22 | 162 | 0.4267 [0.3000, 0.5808] | 0.4181 | 0.4888 [0.3804, 0.5887] | 未驗證（**覆蓋閘門未過**：held-out 162 < 200；探針未偵測記憶、無洩漏降級） |
| `end`（窗口結束） | 26 | 234 | 0.4451 [0.3290, 0.5568] | 0.4284 | 0.5639 [0.5078, 0.6010] | 未驗證（CI 含 0.5；探針下限 > 0.5 → 洩漏降級） |
...
- 三個子集的洩漏探針**各自完整**（`start` n=126、`peak` n=72、`end` n=90，各以該種類的錨點日每隔 2 個取樣），因此 `peak` 的分級理由**只有**覆蓋閘門——它的探針 0.4888 [0.3804, 0.5887] 並未偵測到記憶。反過來說，洩漏訊號集中在 `end`（窗口結束日）與 pooled，這是一個有用的線索：**模型記得的比較像是「2021 某段時間整體的走向」，而窗口結束日的錨點剛好落在那些區段上**；`start` 錨點則完全沒有記憶訊號且方向顯著為負。
...
| anchor 分解的三支探針補打 | 6 | 27,216 | $0.0011 | 次要分析（其餘命中快取） |
| **合計** | – | **3,231,119** | **$0.1357** | – |

- **主配置（H=5）的邊際成本 = $0.0354**（92 requests / 707,999 tokens 判定 + 46 requests / 208,656 tokens 探針，後者含 16 列快取）。三種 anchor 種類的分解與其探針**不再額外花錢**（同一個 run dir 以 `--spec-arg anchor_kind=` 離線重算，補打的只有 6 個新探針 request）。
