...
| 6 | ~~Add `ChannelID` to the 64 BTM tasks that currently fetch via closure~~ | `cmd/atlas/*_tasks.go` | Resolved for Yahoo US market channels: split `us_market_refresh` batch into 8 per-channel BTM tasks (`us_market_refresh_<ch>`), each with `ChannelID`. `macro_ingest` remains the only documented batch closure (multiple macro channels via `IngestAndUpdateMacro`). |
