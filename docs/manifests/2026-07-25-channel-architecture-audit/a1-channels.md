...
`internal/apigateway/CONSTITUTION.md` was revised to **v1.3** on 2026-07-25 and now states **37 channels** plus background-task registration requirements. The `a1-channels.json` / `a2-tasks.json` artifacts in this manifest are the source of truth for the canonical channel list and task registry; they should be regenerated by CI (see checklist item "Proposed channel-index document is auto-generated and CI-enforced").
