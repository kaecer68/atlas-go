...
| SA02 | strategic prior 有 BaseWeights、BaseAllocations、C07 hardcoded 三份 | 沒有單一 L1 prior、provenance 與 calibration state | `internal/config`、`internal/eventdriven/sector_predictor.go`、`internal/sectorallocation`、golden config | C07 20-L1 seed 遷入 ParametersConfig；source/model_version/calibration_status 齊全；所有 consumer 讀同一 prior；duplicate map static search=0 | pending | parameter-system；C07 spec | 初始 seed 為 heuristic/calibrating，只可 dark launch |
| SA03 | legacy BaseAllocations 混合 L1/L2/defensive/cash | 2026-05 config migration 未分層；2026-06 unified migration 未完成 | `internal/config`、`internal/portfolio`、config snapshots/migration tests | L1 legacy 值不覆寫 prior；L2 遷 theme sleeve；cash/defensive 遷 asset/strategy overlay；compat read 有 metric/log；sunset gate 明定 | pending | migration section + config docs | legacy 不再作 missing-sector source |
| SA04 | WeightEngine 輸出非 canonical 12 keys，且與 rotator 重複 normalize/macro | formula 與 final allocation projection owner 不明 | `internal/sectorallocation`、`internal/portfolio`、engine tests | Compute pipeline 只輸出 20 L1；單一 final projection；sum tolerance 1e-9；每個 driver 最多一次；非 canonical 與 projection failure 明確報錯 | pending | WeightEngine contract | 不新增第二套公式 |
| SA05 | E07 PrimaryFlow 空值卻被 F05 plan 當 action | assessment 與 rotator action vocabulary 無 anti-corruption layer；校準尚未完成 | `internal/capitalflow` interface、`internal/orchestrator` adapter、mapping tests | ineligible assessment 不 mutation；無 mapper回 `capital_flow_action_unavailable`；mapper 必須 model-versioned 且通過 walk-forward 才可 eligible | pending | capital-flow spec §13 cross-link | 不把 bullish/bearish 猜成 risk_off 等 action |
| SA06 | WeightEngine 由 Dashboard 擁有；各 System caller wiring 分散 | composition root 缺少 shared dependency owner | `cmd/atlas` composition、`internal/monitoring`、`internal/orchestrator`、wiring tests | engine 建立一次並共享；IndustryService 不建 partial engine；六 constructor matrix 完整；只有四 simulation path 可執行；auto_experiment/live negative tests 綠 | pending | architecture/composition docs | 不從 Dashboard getter 反向取 engine |
| SA07 | `currentSectorAllocations()` 永遠 nil | 沒有 positions→symbol→L1 exposure adapter | `internal/orchestrator`、`internal/portfolio`、industry symbol mapper | current 由 simulation positions/price 計算；sum contract 明確；unknown exposure >0 時 fallback；target 不可當 current | pending | simulation exposure contract | 必須揭露 unmapped symbols/weight |
| SA08 | `ApplySectorRotation()` 回 true 但不修改策略；現行計算位於 session 尾端；CLI simulation 另會把 positions 同步到 live store | strategy evolver 只做 gate，沒有持久化的 next-session allocation policy／allocator consumption；缺 authoritative next-session resolver；simulation/live state 邊界未隔離 | `internal/orchestrator/strategy_evolver.go`、simulation allocator/state、session-date resolver、replay dataset、`cmd/atlas/main.go` live-sync gate、integration tests | day T snapshot 只能產生 `effective_from > as_of` 的 next-session policy；resolver unavailable 時 fail closed；Applied=true 必附 policy hash/receipt；下一有效 session 產單前必須消費 policy；Applied=false 不 mutation；rollback 可重現；policy-enabled CLI simulation 前後 live store bytes 不變 | pending | sim-engine + runbook + simulation/live boundary | 禁止 look-ahead；只存無 consumer snapshot 不算 applied；現行 simulation→live sync 必須移除或獨立 default-off opt-in |
| SA09 | REST/UI/MCP 顯示不同 weight formula，缺 provenance/fallback | dashboard overview、WeightEngine endpoint、MCP passthrough 分裂 | monitoring handlers/service、shared_web、atlas-mcp、contract tests | 同 snapshot target/current/delta/model_version/status/reason 一致；白話 derivation 可見；empty/degraded state 不誤導 | pending | dashboard API contract、tool catalog | 需跨介面 equality test |
| SA10 | F06 strategy ranking 可能使用 synthetic 或空 history | ComparisonEngine 無真實逐策略 attribution 與 benchmark gate | strategy comparison/store、recommender、orchestrator outcome wiring、UI/MCP | non-synthetic replay only；真實 TAIEX benchmark；StrategyID/SessionDate/evaluation_mode 齊全；不足為 warming_up；無 hardcoded ranking | pending | reporting/ranking contract | 可先開發 store，不得先進 observation |
| SA11 | 沒有一致的 dark launch、promotion、rollback、legacy sunset | 過去以 code merged 或局部測試當完成 | feature flags、metrics、observation store、operations runbook | 最少 20 個有效 simulation sessions；invariant violation=0；fallback/差異報告完整；promotion/rollback 演練；legacy read count=0 才 sunset | pending | 新 runbook + observation report | session 不足維持 observing，不得標 done |
| SA12 | 容易遺留 dead config、adapter、假 log 與文件衝突 | close-out 沒有正向＋負向 proof bundle | cleanup、verification script/report、manifest/docs sync | duplicate weights=0；legacy prod callers=0；nil current=0；fake applied=0；live mutation=0；synthetic ranking=0；全域 gates 綠 | pending | verification report；retail manifest F05 done | manifest verifier 與 documentation map 同步 |
...
| Real shadow ranking | SA10 | SA08 outcome attribution available | non-synthetic + benchmark + warming_up | pending | 執行後填入 |
