...
| 8 | `data/state/government_flow/` | 官股行庫代理 | E | `YYYYMMDD.json`（總計）、`YYYYMMDD_brokers.json`（per-broker 明細，PR-A）、`YYYYMMDD_insurance.json`（保險券商）；來源 `bsr.twse.com.tw`（**節律 + CAPTCHA 退避**：BTM `government_flow_aggregate` 每日 15:00+ Taipei 一次 + 連續 CAPTCHA 24h cooldown，fix/20260731-govflow-cadence） |
