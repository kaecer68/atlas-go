...
| 8 | `data/state/government_flow/` | 官股行庫代理 | E | `YYYYMMDD.json`（總計）、`YYYYMMDD_brokers.json`（per-broker 明細，PR-A）、`YYYYMMDD_insurance.json`（保險券商）；來源 `bsr.twse.com.tw`（**節律 + CAPTCHA 退避**：BTM `government_flow_aggregate` 每日 15:00+ Taipei 一次 + 連續 CAPTCHA 24h cooldown，fix/20260731-govflow-cadence）；**替代來源（R5 建議）**：FinMind `TaiwanStockGovernmentBankBuySell`（Sponsor tier，2021-06-30 起，issue #1659 方案 B） |
| 9 | `data/state/sector_index/` | 產業指數報酬 | E | `sector_indices_YYYYMMDD_YYYYMMDD.json`；8/18 產業 schema；`SectorIndexReader` 統一讀取（PR-A） |
| 10 | `data/state/experiments*` | 實驗記錄 | S/E/U | experiments.jsonl + 實驗目錄 + 突變提案 |
