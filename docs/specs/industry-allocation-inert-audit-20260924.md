...
**追蹤（原「未修，需另票」→ 已由 #1987 結案）**：`domain.Quote.Volume` 一個欄位兩種單位的根治 = 在 provider 邊界統一（Fugle×1000、Fubon proxy×1000、Fugle WS×1000），**已由 issue #1987 完成**：`fugle_client.go`／`fubon_client.go`（單筆＋批次）／`realtime/fugle_ws.go` 在邊界 ×`domain.SharesPerLot`，本節所述的消費端單點換算（`quoteVolumeLotSources`／`quoteVolumeInShares`／`LotsConverted`）**已全數刪除**（留著會對已正規化的報價雙重 ×1000）。#1987 同時以生產實測推翻本檔 §9.6 的一個錯誤前提：`fugle_candles` 是**股**不是張（Fugle 官方文件：historical 日K=成交股數；生產 110/110 對 TWSE 精確相符）⇒ 它本來就不該在換算表裡。①`screener` 門檻語意不變（本來就是股語意）；②ledger 歷史**實測全股、零 migration**；③前端「張」顯示維持 ÷1000 在顯示層。tripwire 測試改為逐 provider 邊界契約（`internal/marketdata/provider_volume_contract_test.go`）。
