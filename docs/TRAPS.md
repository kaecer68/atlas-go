...
| **本機 dev 不要 `docker compose up -d fubon-proxy`** | fubonproxy | 若 docker compose 起了 fubon-proxy(在 8081),又 `go run` atlas-go,ProcessManager 看到 port 8081 已有 healthy fubon-proxy → 跳過 spawn(這步是對的);**但**如果 dev 時 docker fubon-proxy 還沒 healthy(例如剛 restart 中),ProcessManager spawn 自己那份 → 8081 EADDRINUSE → supervisor loop。**最簡單**:dev 時 `docker compose up -d postgres redis`(省略 fubon-proxy),完全讓 ProcessManager 管。若 production 部署已驗證 docker fubon-proxy + `host.docker.internal:8081` 通了,維持 docker compose 統一管理也行,但 dev 不要混用。 |
