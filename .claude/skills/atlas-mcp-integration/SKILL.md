...
| Tool 實作 | `cmd/atlas-mcp/server/tools_*.go`（16 個檔案）+ `cmd/atlas-mcp/server/tools.go`（5 個核心 entry-point） | 108 個 tool 的 handler |
...
| Tool 註冊 | `cmd/atlas-mcp/server/tools.go` | `registerAllTools()` |
...
重新啟動 Claude Desktop 後，agent 即可使用 `regime_get_history`、`strategy_list_active` 等 108 個 tool。
...
| 技能 | 關聯 |
|------|------|
| `atlas-mcp-tool-tour` | 108 個 tool 的分群導覽 — 接入成功後的下一步 |
