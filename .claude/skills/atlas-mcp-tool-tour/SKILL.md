...
# Atlas MCP Tool Tour — 117 工具分群導覽

## 描述（Description）

本技能提供 atlas-mcp 的 117 個 MCP tool 的**任務導向分群導覽**。不重複 [`docs/reference/tool-catalog.md`](../../../docs/reference/tool-catalog.md) 的完整 catalog — 本技能聚焦於：
...
- 當 agent 面對 117 個 tool 不知從何開始、需要導覽
- 當 agent 呼叫了一個 tool 但回傳結果不夠、想知道「下一步該用哪個 companion tool」

## 核心概念（Core Concepts）

### 任務群組（Task Domain）
117 個 tool 依任務領域分為 17 個群組（從 31 個 `tools_*.go` 檔案 + `tools.go` 核心 entry-point），每個群組有明確的 **entry-point tool**（第一個該呼叫的）和 **deep-dive tools**（深入查詢用）。
...
| 敘事（Narrative） | 8 | `narrative_get_bundle` | 因果鏈、敘事事件、季節性、briefing bundle |
| 風險（Risk） | 5 | `risk_get_metrics` | 風險聚合、drawdown、相關矩陣、校準 |
| 策略（Strategy） | 7 | `strategy_list_active` | 線上策略、歸因、摘要、層級 |
| 實驗（Experiment） | 5 | `experiment_diff` | 候選 vs baseline 差異、評審、歷史 |
| 達爾文（Synergy） | 3 | `synergy_get_darwinian_status` | 達爾文權重、趨勢、L2.4 觀察窗口 |
| 警報（Alert） | 8 | `alert_list_unacknowledged` | 未確認警報、統計、規則 |
| 控制平面（Control） | 7 | `control_get_active_overrides` | 覆寫狀態、稽核記錄（皆 read-only） |
| 排程（Scheduler/Task） | 4 | `scheduler_get_status` | 背景排程、任務 CRUD |
| 系統健康（System/Health） | 7 | `system_get_health` | 整體健康、LLM router、資料品質、circuit breaker、anomaly 觀測 |
...
| 個股（Stock） | 5 | `stock_get_quote` | 報價、基本面、籌碼、技術、月營收 |
| 標的宇宙（Universe） | 3 | `universe_get_sessions` | simulation session 清單/detail、universe overlap |
...
| 數據 | 模組/檔案 | 說明 |
|------|----------|------|
| Tool 全量定義 | `cmd/atlas-mcp/server/tools_*.go`（31 個檔案）+ `cmd/atlas-mcp/server/tools.go`（核心 entry-point） | 117 個 tool handler |
| 自動描述 | `cmd/atlas-mcp/auto-desc.gen.json`（1146 行） | descgen 生成的 tool description |
| Tool catalog | `docs/reference/tool-catalog.md` | 117 個 tool 的完整清單與決策樹 |
...
| 1.2 | 2026-08-13 | 工具數同步 117/119（PRISM 移除 #1527）、群組計數對齊 tools/list（Alert 8、Strategy 7、Control 7、Experiment 5、System 7、Narrative 8）、補個股/標的宇宙群組、31 個 tools_*.go 檔案 |
