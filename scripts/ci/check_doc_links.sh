...
grep -rnP '\[.*?\]\([^http#].*?\.md[#\)]' docs/ --include="*.md" 2>/dev/null | grep -v ".git/" | while IFS=':' read -r file line content; do
    target=$(echo "$content" | grep -oP '\([^)]+\.md' | tr -d '(' | head -1)
    [[ -z "$target" ]] && continue
...
[ "$FAILED" -eq 1 ] && echo "❌ Broken links found" && exit 1
echo "  ✅ All links OK"
